...
module.exports = class WeatherCommand extends Command {
	...
	async run(msg, args) {
		...
		if (!config.weather.google_token) return msg.reply('my Commander has not set the Google API Key. Go yell at him.');
		if (!config.weather.weather_token) return msg.reply('my Commander has not set the Weather API Key. Go yell at him.');

		const locationURI = encodeURIComponent(location.replace(/ /g, '+'));
		const response = await request({
			uri: `https://maps.googleapis.com/maps/api/geocode/json?address=${locationURI}&key=${config.weather.google_token}`,
			headers: { 'User-Agent': `EscanorBot v${version} (https://github.com/mordose/escanor-bot/)` },
			json: true
		});
		...
		const res = await request({
			uri: `https://api.darksky.net/forecast/${config.weather.weather_token}/${params}?exclude=minutely,hourly,flags&units=auto`,
			headers: { 'User-Agent': `EscanorBot v${version} (https://github.com/mordose/escanor-bot/)` },
			json: true
		});
